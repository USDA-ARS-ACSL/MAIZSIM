...
#define new DEBUG_NEW
	...
	double CGasExchange::EvalCi(double Ci)
	{
		...
		*/
		...
		{
			newCi = std::max(1.0,CO2 - AssimilationNet*(sParms.SC_param/StomatalConductance+ sParms.BLC_param/BoundaryLayerConductance)*(Press/100.0));
		}
		else
			newCi = std::max(1.0,CO2 - AssimilationNet*(sParms.SC_param/eqlTolerance+ sParms.BLC_param/BoundaryLayerConductance)*(Press/100.0));
		return (newCi-Ci);
	}
